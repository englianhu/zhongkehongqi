...
版 <- llply(集, function(迭) {
    牍 = 迭[[1]] # %>% mutate_if(any_vars(!is.numeric(.)), as.numeric)
    ...
    # 牍$`居民储蓄（亿人民币）` <- vector(mode = 'numeric', length = nrow(牍))
    # 牍$`居民储蓄（亿人民币）` <- rep(NA, nrow(牍))
    ...
    # 牍 %<>% mutate(
      # `居民储蓄（亿人民币）` = ifelse(is.null(`居民储蓄（亿人民币）`), NA, `居民储蓄（亿人民币）`), 
    #   居民储蓄 = `居民储蓄（亿人民币）` * 100000000)
    # 牍$`居民储蓄（亿人民币）` <- NULL
    ...
    # 版[grep('(余额*.+（亿人民币）)',  names(版))] %>% names
    # 牍$`本外币余额（亿人民币）` <- vector(mode = 'numeric', length = nrow(牍))
    # 牍$`本外币余额（亿人民币）` <- rep(NA, nrow(牍))
    ...
    # 牍 %<>% mutate(
    #  `本外币余额（亿人民币）` = ifelse(is.null(`本外币余额（亿人民币）`), NA, `本外币余额（亿人民币）`), 
    # 本外币余额 = as.numeric(`本外币余额（亿人民币）`) * 100000000)
    # 牍$`本外币余额（亿人民币）` <- NULL
