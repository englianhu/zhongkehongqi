...
借鉴春秋战国史、七大洲五大洋、秦国China而秦人为Chinese、左氏春秋与战国策分析天下局势（此文章所采撷的范例并无任何天下诸侯详细军事、外交、内阁数据）、秦人牧马论、[马克·棣逊（阳历一九九六年）]()中应用英国数学家罗纳尔·飞雪（渔夫 - 姜太公钓鱼）的大数定律概念部署全局而非局部样本与愚生旧作[猫城@`Rmodel` 赔率建模与测试博彩庄的无效性（英）](https://github.com/englianhu/Rmodel)[^1]，此文章采撷咱们祖国秦国与天下诸侯经济数据为范例，通过统筹法概念和计量经济学来解析。
...
版 <- llply(集, function(迭) {
    ...
    # names(牍) <- gsub('地方财政收入税收收入', '地方财税', names(牍))
    # names(牍[grep('地方', names(牍))])[1] <- '地方财税甲'
    # names(牍[grep('地方', names(牍))])[2] <- '地方财税乙'
    # 牍[grep('地方', names(牍))] %<>% mutate(地方财税 = ifelse(!is.na(地方财税甲), 地方财税甲, 地方财税乙), 地方财税甲 = NULL, 地方财税乙 = NULL)
    牍$地方财税 <- rowSums(牍[grep('地方', names(牍))], na.rm = TRUE) %>% na_if(0)
    ...
    牍$`居民储蓄（亿人民币）` <- NULL
    names(牍) <- gsub('^城乡居民储蓄余额$|居民储蓄存款余额', '居民储蓄', names(牍))
...
names(版[grep('地方', names(版))])[1] <- '地方财税甲'
names(版[grep('地方', names(版))])[2] <- '地方财税乙'
版 %>% 
  mutate(
    地方财税 = ifelse(!is.na(地方财税甲), 地方财税甲, 地方财税乙), 
    地方财税甲 = NULL, 地方财税乙 = NULL)
