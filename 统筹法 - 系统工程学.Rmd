...
借鉴春秋战国史、七大洲五大洋、秦国China而秦人为Chinese、左氏春秋与战国策分析天下局势（此文章所采撷的范例并无任何天下诸侯详细军事、外交、内阁数据）、秦人牧马论、[马克·棣逊（阳历一九九六年）]()中应用英国数学家罗纳尔·飞雪（渔夫 - 姜太公钓鱼）的大数定律概念部署全局而非局部样本与愚生旧作[猫城@`Rmodel` 赔率建模与测试博彩庄的无效性（英）](https://github.com/englianhu/Rmodel)[^1]，此文章以计量经济学（ξconometrics）为主，采撷[聚汇数据](https://www.gotohui.com)咱们祖国秦国与天下诸侯经济数据为范例，通过统筹法概念和计量经济学来解析。三国志中的诸葛孔明在隆中自诩春秋战国时期的管仲乐毅之才，管仲 --- 拮据的经济学管家[JG经管之家](http://bbs.pinggu.org)是个专业且秦国最大的计量经济学论坛。
...
版 <- llply(集, function(迭) {
    ...
    names(牍) <- gsub('\\(人民币|\\(人民币\\)|\\(户', '（人民币）', names(牍))
    ...
    names(牍) <- gsub('农村居民|农牧民|农牧居民|渔农村居', '农林牧渔民', names(牍))
    names(牍) <- gsub('城镇居民', '邑民与农民工', names(牍))
    牍$`农林牧渔民可支配收入（人民币）` <- rowSums(牍[grep('农林牧渔民可支配收入', names(牍))], na.rm = TRUE) %>% na_if(0)
    牍[grep('农林牧渔民可支配收入', names(牍))] <- NULL
    牍$`邑民与农民工人均可支配收入（人民币）` <- rowSums(牍[grep('居民人均可支配收入', names(牍))], na.rm = TRUE) %>% na_if(0)
    牍[grep('居民人均可支配收入', names(牍))] <- NULL
    
    牍$地方财税 <- rowSums(牍[grep('地方|财政', names(牍))], na.rm = TRUE) %>% na_if(0)
    牍[grep('财政', names(牍))] <- NULL
...
由于采撷于[聚汇数据](https://www.gotohui.com)的原数据分类参差不齐，故此需要清洗并分类原数据。以下连接提供一些经济学专用术语：
