...
#define MESSAGE_LINE 6	/* Misc, nessages on ths line */
			   ...
			   int start_row, int start_col)
{
	...
	mvprintw(start_row + current_row + 3, start_col, 
			"Move hightlight then press Return ");
		refresh();
}
...
void remove_cd()
{
	...
	while (fgets(entry, MAX_ENTRY, titles_fp)) {
		...
		if (strncmp(current_cat, entry, cat_length) != 0) fputs(entry, temp_fp);
	}
	fclose(titles_fp);
	fclose(temp_fp);
...
void count_cds()
{
	...
	titles_fp = fopen(title_file, "r");
	if (titles_fp) {
		while (fgets(entry, MAX_ENTRY, titles_fp)) titles++;
		fclose(titles_fp);
	}
	tracks_fp = fopen(tracks_file, "r");
	if (tracks_fp) {
		while (fgets(entry, MAX_ENTRY, tracks_fp)) tracks++;
		fclose(tracks_fp);
	}
...
void find_cd()
{
	...
	if (count != 1) {
		...
		if (count > 1) {
			mvprintw(ERROR_LINE, 0, 
					"Sorry, match is ambigous: %d CDs found. ", count);
		}
		current_cd[0] = '\0';
		get_return();
	}
...
void list_tracks()
{
	...
	/*Make a new pad, ensure that even if there is only a simple
	 ...
	 while (fgets(entry, MAX_ENTRY, tracks_fp)) {
	 	...
	 	/* Compare catalog number number and output rest of entry */
